...
if($nrocli>0){
			...
			$query_despachono = sprintf("SELECT * FROM clientepedido WHERE clientepedido.status=2 AND clientepedido.idcliente=%s",$_POST['cliente']);
		...
		mysql_select_db($database_master, $master);
		$query_despachono = "SELECT * FROM clientepedido WHERE clientepedido.status=2";
		$despachono = mysql_query($query_despachono, $master) or die(mysql_error());
		$row_despachono = mysql_fetch_assoc($despachono);
		$totalRows_despachono = mysql_num_rows($despachono);
}
...
<h1>Pedidos Despachados <?php clie2($nrocli) ?></h1><hr>
<form action="despachadosi.php" method="post">
...
<table class="table table-bordered">
  ...
  ?>
   <?php do {
     ...
     <?php }else{?>
     <tr>
       <td align="center" colspan="5">Cliente No Tiene Pedidos Despachados</td>
      </tr> 
